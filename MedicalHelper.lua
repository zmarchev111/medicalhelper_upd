...
but.select_button			= {false, false, false, false, false, false, false, false, false, true, false}
...
local buf_table_note = {
	[1] = { name = u8("Лист 1"), text = u8("Пустой лист") },
	[2] = { name = u8("Лист 2"), text = u8("Пустой лист") },
	[3] = { name = u8("Лист 3"), text = u8("Пустой лист") },
	[4] = { name = u8("Лист 4"), text = u8("Пустой лист") },
	[5] = { name = u8("Лист 5"), text = u8("Пустой лист") },
	[6] = { name = u8("Лист 6"), text = u8("Пустой лист") },
	[7] = { name = u8("Лист 7"), text = u8("Пустой лист") },
	[8] = { name = u8("Лист 8"), text = u8("Пустой лист") },
	[9] = { name = u8("Лист 9"), text = u8("Пустой лист") },
	[10] = { name = u8("Лист 10"), text = u8("Пустой лист") },
	[11] = { name = u8("Лист 11"), text = u8("Пустой лист") },
	[12] = { name = u8("Лист 12"), text = u8("Пустой лист") },
	[13] = { name = u8("Лист 13"), text = u8("Пустой лист") },
	[14] = { name = u8("Лист 14"), text = u8("Пустой лист") },
	[15] = { name = u8("Лист 15"), text = u8("Пустой лист") },
	[16] = { name = u8("Лист 16"), text = u8("Пустой лист") },
	[17] = { name = u8("Лист 17"), text = u8("Пустой лист") },
	[18] = { name = u8("Лист 18"), text = u8("Пустой лист") },
	[19] = { name = u8("Лист 19"), text = u8("Пустой лист") },
	[20] = { name = u8("Лист 20"), text = u8("Пустой лист") },
}

local buf_rptext = {
	[1] = {
		name = u8("Головная боль"),
	...
	[38] = {
		name = u8("Поправить бейджик"),
		text = u8("{wait:500}\n/me поправил{sex} бейджик\n{wait:1500}\n/do На бейджике: {myTag} | Доктор {myNick} | {myRank}\n{wait:1500}\n/clist {myClist}")
	},
	[39] = {
		name = u8("Выпить таблетку"),
...
function main()
	...
	if doesFileExist(adress.config.."/MedicalHelper/roleplay.json") then
		...
		if f then
			...
			buf_rp.v = rptext[1].text
			buf_rp_name.v = rptext[1].name
		end
		...
		saveData(buf_rptext, "moonloader/config/MedicalHelper/roleplay.json")
		rptext = buf_rptext
		buf_rp.v = buf_rptext[1].text
		buf_rp_name.v = buf_rptext[1].name
	end
	...
	if doesFileExist(adress.config.."/MedicalHelper/note.json") then
		...
		if f then
			...
			buf_note.v = table_note[1].text
			buf_note_name.v = table_note[1].name
		end
		print("{ffffff}Чтение файла {ff2e2e}'note' {ffffff}прошло успешно.")
	else 
		print("{ffffff}Файл {ff2e2e}'note' {ffffff}не найден. Создание файла.")
		saveData(buf_table_note, "moonloader/config/MedicalHelper/note.json")
		table_note = buf_table_note
		buf_note.v = buf_table_note[1].text
		buf_note_name.v = buf_table_note[1].name
	end
	...
	while true do
		...
		wait(0)
		--	takehandle()
			if buffer.Offcl.v == true then imgui_clickmenu() end
			timer()
				...
				imgui.BeginChild('##set', imgui.ImVec2(732, 406), true)
					...
					if but.select_menu[1] then
						...
						imgui.BeginChild('##nhadds', imgui.ImVec2(188, 100), true)
							imgui.CenterText(u8'На данный момент\nтут ничего нет')
						imgui.EndChild()
					end	
									...
									imgui.BeginChild('#cset_1', imgui.ImVec2(181, 359), true)
										imgui.BeginGroup()
											for k, v in pairs(rptext) do
												...
												if imgui.Selectable('  '..u8:decode(u8(('%s'):format(v.name))), but.cset == k) then
													but.cset = k
													buf_rp_name.v = u8:decode(u8(v.name))
													buf_rp.v = u8:decode(u8(v.text))
												end
											end
										imgui.EndGroup()
									imgui.EndChild()
											...
											if but.cset == k then
												...
												imgui.BeginGroup()
													if imgui.Button(u8'Сохранить', imgui.ImVec2(100, 20)) then 
														v.name = u8:decode(u8(buf_rp_name.v))
														v.text = u8:decode(u8(buf_rp.v))
														saveData(rptext, "moonloader/config/MedicalHelper/roleplay.json")
													end
													imgui.SameLine(104)
													if imgui.Button(u8'Сбросить', imgui.ImVec2(100, 20)) then
														v.name = u8:decode(u8(buf_rptext[k].name))
														v.text = u8:decode(u8(buf_rptext[k].text))
														buf_rp_name.v = u8:decode(u8(v.name))
														buf_rp.v = u8:decode(u8(v.text))
														saveData(rptext, "moonloader/config/MedicalHelper/roleplay.json")
													end
								...
								imgui.BeginChild('#notelist', imgui.ImVec2(181, 359), true)
									imgui.BeginGroup()
										for k, v in pairs(table_note) do
											if imgui.Selectable('  '..u8:decode(u8(('%s'):format(v.name))), but.nset == k) then
												but.nset = k
												buf_note_name.v = u8:decode(u8(v.name))
												buf_note.v = u8:decode(u8(v.text))
											end
										end 
									imgui.EndGroup()
								imgui.EndChild()
								...
								imgui.BeginChild('#notetext', imgui.ImVec2(529, 359), true)
									...
									for k, v in pairs(table_note) do
										if but.nset == k then
											...
											imgui.BeginGroup()
												if imgui.Button(u8'Сохранить', imgui.ImVec2(100, 20)) then 
													v.name = u8:decode(u8(buf_note_name.v))
													v.text = u8:decode(u8(buf_note.v))
													saveData(table_note, "moonloader/config/MedicalHelper/note.json")
												end
												imgui.SameLine(104)
												if imgui.Button(u8'Очистить', imgui.ImVec2(100, 20)) then
													v.name = u8:decode(u8(buf_table_note[k].name))
													v.text = u8:decode(u8(buf_table_note[k].text))
													buf_note_name.v = u8:decode(u8(v.name))
													buf_note.v = u8:decode(u8(v.text))
													saveData(table_note, "moonloader/config/MedicalHelper/note.json")
												end
...
function sampev.onServerMessage(color, message)
	...
	if message:match("Медик "..nickname.." вылечил .+") then
		medWork.heal = medWork.heal + 1
	end
	if message:match("Медкарта обновлена") then
		medWork.medcard = medWork.medcard + 1
	end
...
function tags(par)
		...
		par = par:gsub("{RusNick}", tostring(buffer.buf_ru_nick))
		...
		par = par:gsub("{myTag}", tostring(buffer.buf_tag))
		par = par:gsub("{myRank}", tostring(chgName.stats[4]))
...
function doklad()
	if calculateHospital() then
		...
		if calculatePost() then 
			if variable.partner ~= "" then
				if checkOnTag() then
					sampSendChat("/r [" ..u8:decode(buffer.buf_tag).. "] Пост: " ..calculatePost().. ". Напарник: " ..partner.. ". Осмотрено: " ..medWork.heal) 
				else
					sampSendChat("/r Пост: " ..calculatePost().. ". Напарник: " ..variable.partner.. ". Осмотрено: " ..medWork.heal) 
				end
			else
				if checkOnTag() then
					sampSendChat("/r [" ..u8:decode(buffer.buf_tag).. "] Пост: " ..calculatePost().. ". Осмотрено: " ..medWork.heal) 
				else
					sampSendChat("/r Пост: " ..calculatePost().. ". Осмотрено: " ..medWork.heal) 
				end
			end
		else 
			if variable.partner ~= "" then
				if checkOnTag() then
					sampSendChat("/r [" ..u8:decode(buffer.buf_tag).. "] Патруль: " ..calculateCity(2).. " [" ..calculateSquare().. "]. Напарник: " ..variable.partner.. ". Осмотрено: " ..medWork.heal) 
				else
					sampSendChat("/r Патруль: " ..calculateCity(2).. " [" ..calculateSquare().. "]. Напарник: " ..variable.partner.. ". Осмотрено: " ..medWork.heal) 
				end
			else
				if checkOnTag() then
					sampSendChat("/r [" ..u8:decode(buffer.buf_tag).. "] Патруль: " ..calculateCity(2).. " [" ..calculateSquare().. "]. Осмотрено: " ..medWork.heal) 
				else
					sampSendChat("/r Патруль: " ..calculateCity(2).. " [" ..calculateSquare().. "]. Осмотрено: " ..medWork.heal) 
				end
			end
		end
	end
...
function finished()
	if calculateHospital() then
		...
		if calculatePost() then 
			if variable.partner ~= "" then
				if checkOnTag() then
					sampSendChat("/r [" ..u8:decode(buffer.buf_tag).. "] Покинул пост: " ..calculatePost().. ". Напарник: " ..variable.partner.. ". Осмотрено: " ..medWork.heal) 
				else
					sampSendChat("/r Покинул пост: " ..calculatePost().. ". Напарник: " ..variable.partner.. ". Осмотрено: " ..medWork.heal) 
				end
			else
				if checkOnTag() then
					sampSendChat("/r [" ..u8:decode(buffer.buf_tag).. "] Покинул пост: " ..calculatePost().. ". Осмотрено: " ..medWork.heal) 
				else
					sampSendChat("/r Покинул пост: " ..calculatePost().. ". Осмотрено: " ..medWork.heal) 
				end
			end
		else 
			if variable.partner ~= "" then
				if checkOnTag() then
					sampSendChat("/r [" ..u8:decode(buffer.buf_tag).. "] Завершил патруль: " ..calculateCity(2).. ". Напарник: " ..variable.partner.. ". Осмотрено: " ..medWork.heal) 
				else
					sampSendChat("/r Завершил патруль: " ..calculateCity(2).. ". Напарник: " ..variable.partner.. ". Осмотрено: " ..medWork.heal) 
				end
			else
				if checkOnTag() then
					sampSendChat("/r [" ..u8:decode(buffer.buf_tag).. "] Завершил патруль: " ..calculateCity(2).. ". Осмотрено: " ..medWork.heal) 
				else
					sampSendChat("/r Завершил патруль: " ..calculateCity(2).. ". Осмотрено: " ..medWork.heal) 
				end
			end
		end
	end
...
function imgui_clmenu()
	...
	imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.04, 0.04, 0.04, 0.45))
	imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.2, 0.2, 0.2, 0.45))
		...
		imgui.Spacing()
			imgui.BeginGroup()
				...
				if but.select_button[3] then
					CheckName()
					if imgui.Button(fa.ICON_HANDSHAKE_O, imgui.ImVec2(46, 26)) then registerRolePlay(35) end -- приветствие
					imgui.SameLine()
					if imgui.Button(fa.ICON_CHECK, imgui.ImVec2(46, 26)) then registerRolePlay(36) end -- следовать
					imgui.SameLine()
					if imgui.Button(fa.ICON_TIMES, imgui.ImVec2(46, 26)) then registerRolePlay(37) end -- прощание
					if imgui.Button(u8"Поправить бейджик", imgui.ImVec2(142, 26)) then registerRolePlay(38) end
					...
					if imgui.Button(u8"Выпить таблетку", imgui.ImVec2(142, 26)) then registerRolePlay(39) end
					if imgui.Button(u8"< Назад", imgui.ImVec2(142, 26)) then 
				...
				if but.select_button[4] then					
					for k, v in pairs(rptext) do
						if k >= 1 and k < 8 then
							if imgui.Button(u8:decode(u8(v.name)), imgui.ImVec2(142, 26)) then registerRolePlay(k) end
						end
					end
				...
				if but.select_button[6] then
					for k, v in pairs(rptext) do
						if k >= 15 and k < 20 then
							if imgui.Button(u8:decode(u8(v.name)), imgui.ImVec2(142, 26)) then registerRolePlay(k) 
							--[[	if k == 15 then 
									sampAddChatMessage('[MedicalHelper] {ffffff} [Удачно] - Для продолжения операции нажмите « Открытый перелом »', CL.COLOR_RED)
									sampAddChatMessage('[MedicalHelper] {ffffff} [Не удачно] - Для продолжения операции нажмите « Рентген »', CL.COLOR_RED)
								elseif k == 17 then
									sampAddChatMessage('[MedicalHelper] {ffffff} [Удачно] - Для продолжения операции нажмите « Закрытый перелом »', CL.COLOR_RED)
									sampAddChatMessage('[MedicalHelper] {ffffff} [Не удачно] - Для продолжения операции нажмите « Ушиб »', CL.COLOR_RED)
								end ]]
							end
						end
					end
				...
				lua_thread.create(function()
					for line in cmdtext:gmatch('[^\r\n]+') do
						if line:match("{wait:%d+}") then
							...
							sampSendChat(u8:decode(tags(line)))
						end
					end		
				end)
			end
